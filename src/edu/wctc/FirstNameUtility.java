...
class FirstNameUtility {
    ...
    public int encodeFirstName(String firstName, String middleInitial) throws MissingNameException {
        ...
        String upperCaseMiddleInitial = middleInitial.toUpperCase();

        if (firstNameCodeMap.containsKey(upperCaseFirstName)) {
            return firstNameCodeMap.get(upperCaseFirstName) + middleInitialCodeMap.getOrDefault(upperCaseMiddleInitial, 0);
        } else {
            return firstInitialCodeMap.getOrDefault(upperCaseFirstName.substring(0, 1), 0)
                    + middleInitialCodeMap.getOrDefault(upperCaseMiddleInitial, 0);
        }
    }
}
